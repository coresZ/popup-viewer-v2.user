...
## [2.1.2] - 2026-09-21 · 合并自 spike/x-support

### Fixed

- **被引用的帖子是 X 长文时引用卡空白（修复）**：长文帖的正文往往只剩一条 `t.co`（`displayText()` 会去掉它）、封面又不在 `media` 里而在 `attachment` 里，于是旧卡片只剩作者行、既无标题也无图。现在按 `attachment.type === 'article'` 走单独分支：显示「长文」标签 + **文章标题**（截两行）+ 摘要一行，缩略图改用**封面** `attachment.image`；没有标题时退回摘要，再无则显示「（无标题长文）」，不会出现空白卡
- **引用帖（quote）完全没处理（修复）**：数据层此前只解原帖与评论，被引用的帖子躺在 `quoted_status_result` 里没人读，弹窗里就只剩一段孤零零的正文。现在：
  ...
  - 新增 4 条回归断言（解包、可见性包装、无引用为 null、被引用长文带出标题与封面）

## [2.1.1] - 2026-09-21 · 合并自 spike/x-support
...
## [2.1.0] - 2026-09-20 · 合并自 spike/x-support
...
- 本分支为 spike 验证版，已合并回 `main`。方案文档见 `docs/plan-x-graphql.md`，真机验证清单见 `docs/spike-x-support.md`
- 代码来源与 MIT 归属见 `THIRD_PARTY_NOTICES.md`（peek，Copyright (c) 2026 xuntianx）
- 当前限制：**尚未实现** HLS 自适应播放（hls.js，会加约 200KB）；回复仅纯文字（图片/GIF/投票等富回复仍走 X 原生）；翻译默认开启、目标语言固定 `zh-cn`（未做设置项）；评论里的长文只显示紧凑卡片，正文不展开；引用帖的卡片只显示两行正文/标题，完整内容点开新窗口看
