...
- **窗体自由缩放**：拖拽窗体四边/四角任意调整大小（最小 260×200），自定义尺寸按站点记忆；重新点击大小预设即恢复预设尺寸；触屏下四角常驻缩放把手（可在移动端设置关闭）
- **弹窗内前进后退**：底部 ← → 调用弹窗内 iframe 的浏览器原生历史（不影响顶层页面），SPA 站点亦支持；`Alt+←/→` 快捷键
- **锁定窗体 / 惯用手**：锁定后不可拖动缩放；左手模式镜像工具栏、关闭按钮移左上角
- **移动端支持**：可在 Android Firefox/Kiwi、iOS Safari（Tampermonkey/Userscripts 扩展）运行；拖拽/缩放基于 Pointer Events 鼠标触屏通用，触屏下按钮加大到 44px，小屏自动铺近全屏
...
- **明暗主题**：跟随系统或手动指定
- **调试模式**：设置 → 高级 →「调试标记」开关（或 URL 带 `pv2_debug`），排查问题时在页面上显示脚本运行状态/模块加载序号/错误信息，默认关闭零痕迹
